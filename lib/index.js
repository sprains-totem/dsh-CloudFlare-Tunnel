...
// Automatically creates a Cloudflare Tunnel for the DSH Web server, exposes a public URL,
// registers trusted hosts, provides shell environment variables, and manages process lifecycle.
      ...
      const timeout = setTimeout(() => {
        if (!resolved) {
          ...
          if (this.status === 'starting') {
            this.status = 'running'; // May still be starting in background
          }
          resolve(this.url);
        }
      }, 20000);
