...
//    isTrustedApiRequest, and client isLoopback checks without running any reverse proxy.
...
/**
 * 解除 DSH 原生来源限制的核心自修复/补丁引擎。
 * 直接对 dsh-client-connection 的服务端和客户端代码进行精确解禁：
 * - 放行 PRIVILEGED_METHODS（设置读写、模型发现、凭据读取）给受信任的公网隧道；
 * - 放行共享 RPC 拦截器的 loopback 限制；
 * - 修正客户端 isLoopbackHostname 判定，使前端 WebUI 完整拥有 Host 控制权。
 */
function liftDshRestrictions(logger) {
  try {
    ...
    function patchFile(fullPath) {
      try {
        ...
        // 2. 客户端 client.js loopback 判定解除
        if (fullPath.endsWith('client.js') && code.includes('function isLoopbackHostname(hostname) {') && !code.includes('function isLoopbackHostname(hostname) { return true;')) {
          code = code.replace(
        ...
        if (changed) {
          ...
          logger?.info(`[cloudflare-tunnel] Successfully lifted DSH restrictions in: ${fullPath}`);
        }
      } catch (err) {
    ...
    function walkDir(dir) {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          } else if (full.includes('dsh-client-connection') || full.includes('dsh-host-apiproxy')) {
            patchFile(full);
          }
        }
      } catch {}
...
class CloudflareTunnelManager {
  ...
  async start() {
    ...
    this.ctx.logger?.info(`[cloudflare-tunnel] Starting cloudflared (${bin}) pointing directly to DSH (${target})...`);
      ...
      try {
        ...
        const onOutput = (chunk) => {
          ...
          if (match && !this.url) {
            ...
            this.ctx.logger?.info('🔓 Native Origin Restrictions: UNLOCKED');
            this.ctx.logger?.info('===============================================================');

            // 动态将隧道主机名注册到 webRuntime 与 connection 的 trustedHosts 白名单
            try {
